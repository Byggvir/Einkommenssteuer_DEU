...
SQL = paste ( 'select T.Jahr as Jahr, concat("Z",T.Tarifzone) as Tarifzone, E.zvE as zvE, E.zvE * power(1.02,T.Jahr-2007) as zvE2, '
              , ' floor(Steuerbetrag(E.zvE * power(1.02,T.Jahr-2007),T.UEckwert,T.SteuerBeiUE, T.p, T.USatz)) as Steuerbetrag'
...
if ( ! exists( "ESTmI" ) ) {
  ...
  ESTmI %>%  filter ( zvE <= 60000 & zvE %% 5000 == 0 & zvE > 5000| zvE == 200000) %>% ggplot(
    mapping = aes()
  ) +
    ...
    labs(  title = paste( 'Steuerbetrag in % des zu versteuernden Einkommens bei 2 % Inflation' )
           , subtitle = paste( 'Tatsächliche Tarife. 2023 und 2024 aus Eckwerten geschätzt.' )
           , x = 'Jahr'
           , y = 'Steuerbetrag / zvE [%]'
           , colour = 'Tarifzone' ) -> p1
  
  ggsave(  filename = paste( outdir, MyScriptName, '_Inflation.png', sep = '' )
           , plot = p1
           , path = WD
           , device = 'png'
           , bg = "white"
